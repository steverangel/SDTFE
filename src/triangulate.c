...
int triangulate( double *sample_data, int num_samples, int **tetra_data, char *options ) {
    ...
    //char options [ 256 ];
    //sprintf( options, "qhull d Qz" );
    //sprintf( options, "qhull d Qz Qt" );
    //sprintf( options, "qhull d QJ Tv" );
    qh_initflags( options );

    // called after points are defined, dim is 4
    qh_init_B ( sample_data, num_samples, 4, 0 );

    // project points to paraboloid for Delaunay triangulation
    qh_setdelaunay( 4, num_samples, sample_data );
    ...
    *tetra_data = (int*)malloc( num_facets * sizeof( int ) * 8 );
    ...
    FORALLfacets {
        if ( !facet->upperdelaunay ) {
            ...
            //#pragma simd
            for ( k = 0; k < 3; ++k ) {
                ab[ k ] = sample_data[ verts1[ B ] * 4 + k ] - sample_data[ verts1[ A ] * 4 + k ];
                ac[ k ] = sample_data[ verts1[ C ] * 4 + k ] - sample_data[ verts1[ A ] * 4 + k ];
                ad[ k ] = sample_data[ verts1[ D ] * 4 + k ] - sample_data[ verts1[ A ] * 4 + k ];
            }
        ...
        if ( !facet->upperdelaunay ) {
            ...
            FOREACHridge_( facet->ridges ) {
                ...
                if ( !neighbor->upperdelaunay ) {
                    ...
                    //assert( comp[ 0 ] + comp[ 1 ] + comp[ 2 ] + comp[ 3 ] == 3);
...
void pre_compute_vol( double **sample_data, int num_samples, int *tetra_data, int num_tetra, float p_mass ) {
    ...
    for ( i = 0; i < num_tetra; ++i ) {
        vol = ( 
                ( (*sample_data)[  tetra_data[ i * 8 + A ] * 4 + 0 ]  - (*sample_data)[  tetra_data[ i * 8 + D ] * 4 + 0 ]  ) *
                (
                    ( (*sample_data)[  tetra_data[ i * 8 + B ] * 4 + 1 ] - (*sample_data)[  tetra_data[ i * 8 + D ] * 4 + 1 ] ) *
                    ( (*sample_data)[  tetra_data[ i * 8 + C ] * 4 + 2 ] - (*sample_data)[  tetra_data[ i * 8 + D ] * 4 + 2 ] ) - 
                    ( (*sample_data)[  tetra_data[ i * 8 + B ] * 4 + 2 ] - (*sample_data)[  tetra_data[ i * 8 + D ] * 4 + 2 ] ) *
                    ( (*sample_data)[  tetra_data[ i * 8 + C ] * 4 + 1 ] - (*sample_data)[  tetra_data[ i * 8 + D ] * 4 + 1 ] ) 
                ) +
                ( (*sample_data)[  tetra_data[ i * 8 + A ] * 4 + 1 ]  - (*sample_data)[  tetra_data[ i * 8 + D ] * 4 + 1 ]  ) *
                (
                    ( (*sample_data)[  tetra_data[ i * 8 + B ] * 4 + 2 ] - (*sample_data)[  tetra_data[ i * 8 + D ] * 4 + 2 ] ) *
                    ( (*sample_data)[  tetra_data[ i * 8 + C ] * 4 + 0 ] - (*sample_data)[  tetra_data[ i * 8 + D ] * 4 + 0 ] ) - 
                    ( (*sample_data)[  tetra_data[ i * 8 + B ] * 4 + 0 ] - (*sample_data)[  tetra_data[ i * 8 + D ] * 4 + 0 ] ) *
                    ( (*sample_data)[  tetra_data[ i * 8 + C ] * 4 + 2 ] - (*sample_data)[  tetra_data[ i * 8 + D ] * 4 + 2 ] )
                ) +
                ( (*sample_data)[  tetra_data[ i * 8 + A ] * 4 + 2 ]  - (*sample_data)[  tetra_data[ i * 8 + D ] * 4 + 2 ]  ) *
                (
                    ( (*sample_data)[  tetra_data[ i * 8 + B ] * 4 + 0 ] - (*sample_data)[  tetra_data[ i * 8 + D ] * 4 + 0 ] ) *
                    ( (*sample_data)[  tetra_data[ i * 8 + C ] * 4 + 1 ] - (*sample_data)[  tetra_data[ i * 8 + D ] * 4 + 1 ] ) - 
                    ( (*sample_data)[  tetra_data[ i * 8 + B ] * 4 + 1 ] - (*sample_data)[  tetra_data[ i * 8 + D ] * 4 + 1 ] ) *
        ...
        (*sample_data)[  tetra_data[ i * 8 + A ] * 4 + 3 ] += vol;
        (*sample_data)[  tetra_data[ i * 8 + B ] * 4 + 3 ] += vol;
        (*sample_data)[  tetra_data[ i * 8 + C ] * 4 + 3 ] += vol;
        (*sample_data)[  tetra_data[ i * 8 + D ] * 4 + 3 ] += vol;
    }
 
    for ( i = 0; i < num_samples; ++i )
       (*sample_data)[ i * 4 + 3 ] = p_mass * ( 1 + DIM ) / (*sample_data)[ i * 4 + 3 ]; 
}
...
void pre_compute_gradients( int *tetra_data, int num_tetra, double *sample_data, double **nabla ) {
    ...
    for ( i = 0; i < num_tetra; ++i ) {
        D_xb = sample_data[ tetra_data[ i * 8 + B ] * 4 + 0 ] - sample_data[ tetra_data[ i * 8 + A ] * 4 + 0 ];
        D_yb = sample_data[ tetra_data[ i * 8 + B ] * 4 + 1 ] - sample_data[ tetra_data[ i * 8 + A ] * 4 + 1 ];
        D_zb = sample_data[ tetra_data[ i * 8 + B ] * 4 + 2 ] - sample_data[ tetra_data[ i * 8 + A ] * 4 + 2 ];
        D_xc = sample_data[ tetra_data[ i * 8 + C ] * 4 + 0 ] - sample_data[ tetra_data[ i * 8 + A ] * 4 + 0 ];
        D_yc = sample_data[ tetra_data[ i * 8 + C ] * 4 + 1 ] - sample_data[ tetra_data[ i * 8 + A ] * 4 + 1 ];
        D_zc = sample_data[ tetra_data[ i * 8 + C ] * 4 + 2 ] - sample_data[ tetra_data[ i * 8 + A ] * 4 + 2 ];
        D_xd = sample_data[ tetra_data[ i * 8 + D ] * 4 + 0 ] - sample_data[ tetra_data[ i * 8 + A ] * 4 + 0 ];
        D_yd = sample_data[ tetra_data[ i * 8 + D ] * 4 + 1 ] - sample_data[ tetra_data[ i * 8 + A ] * 4 + 1 ];
        D_zd = sample_data[ tetra_data[ i * 8 + D ] * 4 + 2 ] - sample_data[ tetra_data[ i * 8 + A ] * 4 + 2 ];

        D_rb = sample_data[ tetra_data[ i * 8 + B ] * 4 + 3 ] - sample_data[ tetra_data[ i * 8 + A ] * 4 + 3 ];
        D_rc = sample_data[ tetra_data[ i * 8 + C ] * 4 + 3 ] - sample_data[ tetra_data[ i * 8 + A ] * 4 + 3 ];
        D_rd = sample_data[ tetra_data[ i * 8 + D ] * 4 + 3 ] - sample_data[ tetra_data[ i * 8 + A ] * 4 + 3 ];
...
int find_forward_hull( int *tetra_data, int num_tetra, double *sample_data, int num_samples, char *fname, word_t *bitmap ) {
            ...
            if ( neighbor_id == -1 ) {
                switch ( j ) {
                    case ABC:
                        //#pragma simd
                        for ( k = 0; k < 3; ++k ) {
                            t1[ k ] = sample_data[ tetra_data[ i * 8 + B ] * 4 + k ] - sample_data[ tetra_data[ i * 8 + A ] * 4 + k ];
                            t2[ k ] = sample_data[ tetra_data[ i * 8 + C ] * 4 + k ] - sample_data[ tetra_data[ i * 8 + A ] * 4 + k ];
                        }
                        break;
                    case ADB:
                        //#pragma simd
                        for ( k = 0; k < 3; ++k ) {
                            t1[ k ] = sample_data[ tetra_data[ i * 8 + D ] * 4 + k ] - sample_data[ tetra_data[ i * 8 + A ] * 4 + k ];
                            t2[ k ] = sample_data[ tetra_data[ i * 8 + B ] * 4 + k ] - sample_data[ tetra_data[ i * 8 + A ] * 4 + k ];
                        }
                        break;
                    case ACD:
                        //#pragma simd
                        for ( k = 0; k < 3; ++k ) {
                            t1[ k ] = sample_data[ tetra_data[ i * 8 + C ] * 4 + k ] - sample_data[ tetra_data[ i * 8 + A ] * 4 + k ];
                            t2[ k ] = sample_data[ tetra_data[ i * 8 + D ] * 4 + k ] - sample_data[ tetra_data[ i * 8 + A ] * 4 + k ];
                        }
                        break;
                    case BDC:
                        //#pragma simd
                        for ( k = 0; k < 3; ++k ) {
                            t1[ k ] = sample_data[ tetra_data[ i * 8 + D ] * 4 + k ] - sample_data[ tetra_data[ i * 8 + B ] * 4 + k ];
                            t2[ k ] = sample_data[ tetra_data[ i * 8 + C ] * 4 + k ] - sample_data[ tetra_data[ i * 8 + B ] * 4 + k ];
                        }
                        break;
                    default:
...
void copy_triangles( int *tetra_data, int num_tetra, double *sample_data, int num_samples, int *tri_data, int num_tri, word_t *bitmap ) {
